...
const ship = (type) => {
    ...
    const hit = (i) => (hits[i] = "hit");  // hit(0) => ["hit", null, null] (this "accesses" `hits` to set a new value) (!!! Rename 'hits' to something that makes more sense)
    const getHits = () => hits;  // accesses and returns `hits`

    // isSunk
    const isSunk = () => hits.every((hit) => hit === "hit");  // ["hit", "hit", "hit"] => true; ["hit", null, "hit"] => false
    // a helper function, that is a generalized version of isSunk, that works for 1 specific index
...
/* (!A combination of lines 16 and 20!) */
